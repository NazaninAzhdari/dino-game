...
entity draw_dino is
    port (
        ...
        i_alive_en  :   in      STD_LOGIC;
        i_run1_en   :   in      STD_LOGIC;
        i_run2_en   :   in      STD_LOGIC;
        i_dead_en   :   in      STD_LOGIC;
...
architecture RTL of draw_dino is
    ...
    ---------------------------------------------------------
    r_alive_dino <= pf_alive(r_y - i_y_dino, r_x - pc_X_DINO) 
                    when (r_x>= pc_X_DINO and r_x<= pc_X_DINO + pc_DINO_SIZE) 
                    and  (r_y>= r_y_dino  and r_y<= r_y_dino  + pc_DINO_SIZE) else '0';

    r_run1_dino  <= pf_run1(r_y - i_y_dino, r_x - pc_X_DINO) 
                    when (r_x>= pc_X_DINO and r_x<= pc_X_DINO + pc_DINO_SIZE) 
                    and  (r_y>= r_y_dino  and r_y<= r_y_dino  + pc_DINO_SIZE) else '0';
    
    r_run2_dino  <= pf_run2(r_y - i_y_dino, r_x - pc_X_DINO) 
                    when (r_x>= pc_X_DINO and r_x<= pc_X_DINO + pc_DINO_SIZE) 
                    and  (r_y>= r_y_dino  and r_y<= r_y_dino  + pc_DINO_SIZE) else '0';

    r_dead_dino <= pf_dead(r_y - i_y_dino, r_x - pc_X_DINO) 
                    when (r_x>= pc_X_DINO and r_x<= pc_X_DINO + pc_DINO_SIZE) 
                    and  (r_y>= r_y_dino  and r_y<= r_y_dino  + pc_DINO_SIZE) else '0';                
